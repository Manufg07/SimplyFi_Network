...
export ORDERER_CA=${PWD}/organizations/ordererOrganizations/simplyfi.com/orderers/orderer.simplyfi.com/msp/tlscacerts/tlsca.simplyfi.com-cert.pem

export ORDERER_ADMIN_TLS_SIGN_CERT=${PWD}/organizations/ordererOrganizations/simplyfi.com/orderers/orderer.simplyfi.com/tls/server.crt

export ORDERER_ADMIN_TLS_PRIVATE_KEY=${PWD}/organizations/ordererOrganizations/simplyfi.com/orderers/orderer.simplyfi.com/tls/server.key
...
peer channel fetch config ${PWD}/channel-artifacts/config_block.pb -o localhost:7050 --ordererTLSHostnameOverride orderer.simplyfi.com -c $CHANNEL_NAME --tls --cafile $ORDERER_CA
sleep 1
...
jq '.channel_group.groups.Application.groups.Org1MSP.values += {"AnchorPeers":{"mod_policy": "Admins","value":{"anchor_peers": [{"host": "peer0.org1.simplyfi.com","port": 7051}]},"version": "0"}}' config_copy.json > modified_config.json
...
peer channel update -f ${PWD}/channel-artifacts/config_update_in_envelope.pb -c $CHANNEL_NAME -o localhost:7050  --ordererTLSHostnameOverride orderer.simplyfi.com --tls --cafile $ORDERER_CA
sleep 1
...
peer lifecycle chaincode install simplyfi.tar.gz
sleep 3

peer lifecycle chaincode queryinstalled
sleep 1

export CC_PACKAGE_ID=$(peer lifecycle chaincode calculatepackageid simplyfi.tar.gz)

echo "—---------------Approve chaincode in Org1 peer—-------------"

peer lifecycle chaincode approveformyorg -o localhost:7050 --ordererTLSHostnameOverride orderer.simplyfi.com --channelID $CHANNEL_NAME --name KBA-Automobile --version 1.0  --package-id $CC_PACKAGE_ID --sequence 1 --tls --cafile $ORDERER_CA --waitForEvent
sleep 2
...
jq '.channel_group.groups.Application.groups.Org2MSP.values += {"AnchorPeers":{"mod_policy": "Admins","value":{"anchor_peers": [{"host": "peer0.org2.simplyfi.com","port": 9051}]},"version": "0"}}' config_copy.json > modified_config.json
...
peer lifecycle chaincode commit -o localhost:7050 --ordererTLSHostnameOverride orderer.simplyfi.com --channelID $CHANNEL_NAME --name KBA-Automobile --version 1.0 --sequence 1 --tls --cafile $ORDERER_CA --peerAddresses localhost:7051 --tlsRootCertFiles $ORG1_PEER_TLSROOTCERT --peerAddresses localhost:9051 --tlsRootCertFiles $ORG2_PEER_TLSROOTCERT
sleep 1
